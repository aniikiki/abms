package com.aniikiki.abms.constant;

public class ResultMessage {

    public static final String SYS_USER_NOT_FOUND = "用户不存在";

    public static final String SYS_ROLE_NOT_FOUND = "角色不存在";

    public static final String SYS_MENU_NOT_FOUND = "菜单不存在";

    public static final String SYS_CONTACT_NOT_FOUND = "联系人不存在";

    public static final String SYS_CONTACT_GROUP_NOT_FOUND = "联系人群组不存在";

    public static final String SYS_UPDATE_PASSWORD_OLDPWD_ERR = "旧密码错误";

    public static final String SYS_USERNAME_ALREADY_EXISTS = "用户名已存在";

    public static final String SYS_ROLE_NAME_ALREADY_EXISTS = "角色名已存在";

}
